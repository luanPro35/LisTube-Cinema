...
import { videoShort } from "../data/VideoShortdata";
import useDragScroll from "../hooks/useDragScroll"; // Import the custom hook

const VideoShort = () => {
  // Use the custom hook to get drag-to-scroll functionality
  const {
    scrollContainerRef,
    handleMouseDown,
    handleMouseLeave,
  ...
  return (
    <div className="mx-auto px-6 py-10 bg-black">
      ...
      >
        {videoShort.map((video) => (
          <div
            key={video.id}
            className="group bg-white dark:bg-gray-900 rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-500 border border-gray-200 dark:border-gray-800 flex-shrink-0 w-64 transform hover:-translate-y-2"
          >
            {/* Image Container */}
